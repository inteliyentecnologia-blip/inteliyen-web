...
export function Header() {
  ...
  // --- LA MAGIA PARA VOLVER AL INICIO ---
  // Esta función te lleva a la ruta "/" y te avienta hasta arriba de la página suavemente
  const goHome = () => {
    setIsMenuOpen(false); // Cierra el menú de celular por si estaba abierto
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-black/80 backdrop-blur-xl border-b border-purple-500/10">
      <nav className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between relative z-50">
        ...
        {/* Logo (También te regresa al inicio y arriba) */}
        <Link to="/" onClick={goHome} className="flex items-center gap-4">
        ...
        <div className="hidden md:flex items-center gap-8 text-sm font-medium text-gray-300">
          {/* Aquí aplicamos la función goHome */}
          <Link to="/" onClick={goHome} className="hover:text-purple-400 transition-colors">INICIO</Link>
      ...
      {isMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-black/95 backdrop-blur-3xl border-b border-purple-500/20 shadow-2xl">
          <div className="flex flex-col p-6 space-y-2 text-center text-lg font-medium text-gray-300">
            {/* Aquí aplicamos la función goHome también */}
            <Link to="/" onClick={goHome} className="block py-3 hover:text-purple-400 transition-colors">INICIO</Link>
