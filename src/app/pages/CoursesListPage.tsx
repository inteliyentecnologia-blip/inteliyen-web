...
const getCardStyle = (id: number) => {
  const styles: Record<number, any> = {
    ...
    5: { text: 'text-[#10B981]', border: 'border-[#10B981]/40', glow: 'shadow-[0_0_20_rgba(16,185,129,0.2)]' },
  ...
  return (
    <div className="min-h-screen bg-black text-white py-20 px-6">
      ...
      <div className="text-center mb-16 px-4">
        <h1 className="text-5xl md:text-7xl flex items-center justify-center gap-4 uppercase">
          <span>📖</span>
        ...
        {coursesData.map((course) => {
          ...
          return (
            <div key={course.id} className={`bg-[#050505] border ${style.border} rounded-3xl p-8 flex flex-col transition-all duration-300 ${style.glow} hover:scale-[1.02]`}>
              ...
              {/* QUITAMOS EL font-bold PARA QUE ENTRE TU TIPOGRAFÍA WIDE */}
              <h2 className={`text-2xl mb-4 ${style.text} uppercase`}>
                {course.title}
              </h2>
              ...
              <div className="flex flex-wrap gap-2 mb-8">
                <span className="px-3 py-1 rounded-full border border-purple-500/30 bg-purple-500/10 text-purple-300 text-[10px] uppercase">
                  {course.audienceType}
                </span>
                <span className="px-3 py-1 rounded-full border border-cyan-500/30 bg-cyan-500/10 text-cyan-400 text-[10px] uppercase">
                  {course.duration}
                </span>
              </div>

              <div className="mt-auto pt-6 border-t border-gray-900">
                <Link to={`/cursos/${course.id}`} className="flex items-center justify-between text-xs uppercase text-gray-500 hover:text-white transition-colors">
                  Ver temario
