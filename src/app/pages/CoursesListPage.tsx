        ...
        {coursesData.map((course) => {
          ...
          return (
            <div key={course.id} className={`bg-[#050505] border ${style.border} rounded-3xl p-10 flex flex-col transition-all duration-500 ${style.glow} hover:-translate-y-2`}>
              ...
              <div className="flex flex-wrap gap-3 mb-10 font-bold text-[10px] uppercase tracking-widest">
                <span className="px-4 py-1.5 rounded-full border border-gray-800 bg-gray-900/50 text-gray-300">{course.audienceType}</span>
                <span className="px-4 py-1.5 rounded-full border border-cyan-500/30 bg-cyan-500/10 text-cyan-400">{course.duration}</span>
              </div>
